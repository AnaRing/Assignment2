...
class Product {
    constructor(name, manufacturer, expiryDate, quantity, type) {
        this.name = name;
        this.productID = Date.now(); // Generating auto ID
...
class LiquidProduct extends Product {
    constructor(name, manufacturer, expiryDate, quantity) {
        super(name, manufacturer, expiryDate, quantity, 'Liquid'); // Set type to 'Liquid'
...
deleteButtons.forEach(button => {
    button.addEventListener('click', () => {
        const productId = button.getAttribute('data__id');
        Product.deleteProduct(productId);
        renderProducts(); // Update the displayed list after deleting a product
    });
});
